		...
		<header className="header w-full bg-white sticky top-0 z-50 border-b-4 border-red-600">
			<div className="container m-auto px-4 h-14 flex justify-between items-center">
				<div className="logo flex items-center">
					<Link className="flex items-center" href="/">
						<span className="text-2xl font-bold text-black">
							<Image src="/logo-horizontal.png" alt="" width={150} height={100} style={{ position: 'relative', left: '-12px' }} />
						</span>
					</Link>
				</div>
				<nav className="hidden md:flex flex-1 justify-center items-center h-full">
					{menuItems.length > 0 && (
						<ul className="nav menu flex list-none gap-8 text-sm font-medium h-full items-center">
				...
				<div className="flex items-center gap-4 h-full">
					...
					<Link className="hidden md:block p-2 hover:bg-gray-100 rounded-full transition-colors" href="/post/search?query=">
						<svg className="w-5 h-5 text-gray-700" fill="none" stroke="currentColor" viewBox="0 0 24 24">
			...
			{showMarquee && <CategoriesMarquee categories={categories} />}
		</header>
	);
}
